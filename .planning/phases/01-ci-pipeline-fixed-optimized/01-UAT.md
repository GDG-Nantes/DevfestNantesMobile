...
updated: 2026-09-17T14:26:49Z
---

## Current Test

number: 1
name: Trigger a real pull_request event against android.yml and inspect the read-only cache behavior
expected: |
  Each job's Setup Gradle summary shows cache-read-only behavior (restore only, no 'Saved cache entry'
  lines), and all four jobs (checks, unit-tests, instrumentation-tests, build-debug) conclude
  successfully — proving the branch-aware expression evaluates correctly under a real pull_request
  event, not just by static YAML inspection.
